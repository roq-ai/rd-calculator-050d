...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    const body = { ...req.body };
    if (body?.recurring_deposit?.length > 0) {
      const create_recurring_deposit = body.recurring_deposit;
      body.recurring_deposit = {
        create: create_recurring_deposit,
      };
    } else {
      delete body.recurring_deposit;
    }
